...
history <- dl_model %>% fit(
  as.matrix(dl_train_norm),
  as.matrix(dl_train_output),
...
test_results <- dl_model %>% evaluate(
  as.matrix(dl_test_norm),
  as.matrix(dl_test_output),
  verbose = 0
)


test_predictions <- predict(dl_model, as.matrix(dl_test_norm))

ggplot(data.frame(pred = as.numeric(test_predictions), concentration = dl_test_output)) +
  geom_point(aes(x = pred, y = concentration)) +
