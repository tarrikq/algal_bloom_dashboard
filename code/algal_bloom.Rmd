...
url6 <- "https://seagull-erddap.glos.org/erddap/tabledap/obs_117.htmlTable?time%2Cnitrate&time%3E=2022-08-04T00%3A00%3A00Z&time%3C=2022-11-01T00%3A00%3A00Z"
wiki_url6 <- read_html(url6) # UCLA SOCR Data
df6 <- as.data.frame(html_table(html_nodes(wiki_url6, "table")[[2]]))

df6 <- df5[-1 , ]

time <- df6$time
df6 <- subset(df6, select=-c(time))
df6 <- as.data.frame(sapply(df6, as.numeric))
df6 <- cbind(time, df6)
df6$time <- ymd_hms(df6$time )

nitrate <- inner_join(df4, df5, by="time")
nitrate <- inner_join(nitrate, df6, by="time")
nitrate$nitrate.y[3316:10860] <- nitrate$nitrate.x[3316:10860]
nitrate <- nitrate %>% select(c("time","nitrate.y"))
...
Amelia will create copies of the dataset with new imputed values. The number of copies created will depend on the value for "m" entered. The copy datasets can then be averaged to determine a mean and variance for the imputed values. 
...
df <- data.frame(val = t, pred = algae_sub$nitrate_ug.L[529:629])

ggplot(df) +
  geom_density(aes(x=pred, fill="Predicted"), alpha=0.5)+
  geom_density(aes(x=nitrate,fill="Observed"), alpha=0.5)+
